...
Вы можете использовать значение "source" как метаданные, вернув следующее:
\`\`\`json
{
    "source": "/source"
}
...
class Jsonlines_DocumentLoaders implements INode {
    ...
    constructor() {
        this.label = 'JSON Lines файл'
        this.name = 'jsonlinesFile'
        this.version = 3.0
        this.type = 'Document'
        this.icon = 'jsonlines.svg'
        this.category = 'Document Loaders'
        this.description = `Загрузка данных из JSON Lines файлов`
        this.baseClasses = [this.type]
        this.inputs = [
            {
                label: 'JSON Lines файл',
                name: 'jsonlinesFile',
                type: 'file',
                fileType: '.jsonl'
            },
            ...
            {
                label: 'Указатель извлечения',
                name: 'pointerName',
                type: 'string',
                placeholder: 'key',
                description:
                    'Пример: { "key": "value" }, Указатель извлечения = "key", "value" будет извлечен как содержимое страницы фрагмента',
                optional: false
            },
            {
