...
query({"question": "Привет, как дела?"}).then((response) => {
...
`
            ...
            return `curl ${baseURL}/api/v1/prediction/${dialogProps.chatflowid} \\
     -X POST \\
     -d '{"question": "Привет, как дела?", "overrideConfig": {${getConfigExamplesForCurl(configData, 'json')}}' \\
     -H "Content-Type: application/json"`
        }
        return ''
    }
                        ...
                        {codeLang !== 'Встраивание' && codeLang !== 'Поделиться чатботом' && codeLang !== 'Configuration' && (
                            <>
                                ...
                                {checkboxVal && getConfigApi.data && getConfigApi.data.length > 0 && (
                                    <>
                                        <Typography sx={{ mt: 2 }}>
                                            Вы можете переопределить существующую конфигурацию ввода агента с помощью свойства
                                            overrideConfig.
                                        </Typography>
                                        <div
                                            style={{
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <span style={{ color: 'rgb(116,66,16)', marginLeft: 10, fontWeight: 500 }}>
                                                    {
                                                        'По соображениям безопасности, переопределение конфигурации отключено по умолчанию. Вы можете изменить это, перейдя в Конфигурацию агента -> вкладка Безопасность, и включить свойство, которое хотите переопределить.'
                                                    }
                                                    &nbsp;Подробнее{' '}
                                                    <a
                                                        rel='noreferrer'
