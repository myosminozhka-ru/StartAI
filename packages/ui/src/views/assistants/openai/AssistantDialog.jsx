...
const AssistantDialog = ({ show, dialogProps, onCancel, onConfirm, setError }) => {
    ...
    useEffect(() => {
        if (getAssistantObjApi.error) {
            ...
            enqueueSnackbar({
                message: `Не удалось получить ассистента: ${errMsg}`,
                options: {
    ...
    const onEditAssistantVectorStoreClick = (vectorStoreObject) => {
        const dialogProp = {
            title: `Редактировать ${vectorStoreObject.name ? vectorStoreObject.name : vectorStoreObject.id}`,
            type: 'EDIT',
    ...
    const onAddAssistantVectorStoreClick = () => {
        const dialogProp = {
            title: 'Добавить векторное хранилище',
            type: 'ADD',
            cancelButtonName: 'Отмена',
    ...
    const addNewAssistant = async () => {
        ...
        try {
            ...
            enqueueSnackbar({
                message: `Не удалось добавить нового ассистента: ${
    ...
    const onDeleteClick = () => {
        setDeleteDialogProps({
            ...
            description: `Выберите метод удаления для ${assistantName}`,
            cancelButtonName: 'Отмена'
        })
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
                    ...
                    <Box>
                        ...
                        <Dropdown
                            ...
                            value={assistantModel ?? 'выберите опцию'}
                        />
                    </Box>
                    <Box>
                        <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                            <Typography variant='overline'>Название ассистента</Typography>
                            <TooltipWithParser title={'Название ассистента. Максимальная длина - 256 символов.'} />
                        </Stack>
                        <OutlinedInput
                            id='assistantName'
                            type='string'
                            size='small'
                            fullWidth
                            placeholder='Мой новый ассистент'
                            value={assistantName}
                            name='assistantName'
                    ...
                    <Box>
                        <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                            ...
                            <TooltipWithParser title={'Описание ассистента. Максимальная длина - 512 символов.'} />
                        </Stack>
                        <OutlinedInput
                            id='assistantDesc'
                            type='string'
                            size='small'
                            fullWidth
                            placeholder='Описание того, что делает ассистент'
                            multiline={true}
                            rows={3}
                            value={assistantDesc}
                            name='assistantDesc'
                    ...
                    <Box>
                        <Stack sx={{ position: 'relative' }} direction='row'>
                            <Typography variant='overline'>Источник иконки ассистента</Typography>
                        </Stack>
                        <div
                            style={{
                                width: 100,
                    ...
                    <Box>
                        <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                            <Typography variant='overline'>Инструкции ассистента</Typography>
                            <TooltipWithParser
                                title={'Системные инструкции, которые использует ассистент. Максимальная длина - 32768 символов.'}
                            />
                        </Stack>
                        <OutlinedInput
                            ...
                            size='small'
                            fullWidth
                            placeholder='Вы - личный репетитор по математике. Когда задают вопрос, напишите и запустите Python код, чтобы ответить на вопрос.'
                            multiline={true}
                            rows={3}
                    ...
                    <Box>
                        <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                            ...
                            <TooltipWithParser
                                title={
                                    'Контролирует случайность: уменьшение приводит к менее случайным ответам. По мере приближения температуры к нулю, модель становится детерминированной и повторяющейся.'
                                }
                            />
                        </Stack>
                        <OutlinedInput
                    ...
                    <Box>
                        <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                            <Typography variant='overline'>Top P ассистента</Typography>
                            <TooltipWithParser
                                title={
                                    'Контролирует разнообразие через ядерную выборку: 0.5 означает, что рассматривается половина всех вариантов с учетом вероятности.'
                                }
                            />
                        </Stack>
                        <OutlinedInput
                        ...
                        <>
                            <Box>
                                <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                                    ...
                                    <TooltipWithParser title='Список инструментов, включенных для ассистента. Максимум 128 инструментов на ассистента.' />
                                </Stack>
                                <MultiDropdown
                                    ...
                                    value={assistantTools ?? 'выберите опцию'}
                                />
                            </Box>
                            <Box>
                                {assistantTools?.length > 0 && assistantTools.includes('code_interpreter') && (
                                    <Card sx={{ mb: 2, border: '1px solid #e0e0e0', borderRadius: `${customization.borderRadius}px` }}>
                                        <CardContent>
                                            <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                                                <Typography variant='overline'>Файлы интерпретатора кода</Typography>
                                                <TooltipWithParser title='Интерпретатор кода позволяет ассистенту писать и запускать код. Этот инструмент может обрабатывать файлы с различными данными и форматированием, а также генерировать файлы, такие как графики' />
                                            </Stack>
                                            ...
                                            <File
                                                ...
                                                fileType='*'
                                                formDataUpload={true}
                                                value={uploadCodeInterpreterFiles ?? 'Выберите файл для загрузки'}
                                ...
                                {assistantTools?.length > 0 && assistantTools.includes('file_search') && (
                                    <Card sx={{ mb: 2, border: '1px solid #e0e0e0', borderRadius: `${customization.borderRadius}px` }}>
                                        <CardContent>
                                            <Stack sx={{ position: 'relative', alignItems: 'center' }} direction='row'>
                                                <Typography variant='overline'>Файлы поиска</Typography>
                                                <TooltipWithParser title='Поиск файлов позволяет ассистенту использовать знания из файлов, которые вы или ваши пользователи загружают. После загрузки файла ассистент автоматически решает, когда извлекать контент на основе запросов пользователя' />
                                            </Stack>
                                            ...
                                            {!toolResources.file_search || !toolResources.file_search?.vector_store_ids?.length ? (
                                                ...
                                                <File
                                                    ...
                                                    fileType='*'
                                                    formDataUpload={true}
                                                    value={uploadVectorStoreFiles ?? 'Выберите файл для загрузки'}
