...
const ShowStoredChunks = () => {
    ...
    const onChunkEdit = async (newPageContent, newMetadata, chunk) => {
        ...
        try {
            ...
            if (editResp.data) {
                enqueueSnackbar({
                    message: 'Фрагмент документа успешно отредактирован!',
                    options: {
            ...
            setLoading(false)
            enqueueSnackbar({
                message: `Не удалось отредактировать фрагмент: ${
    ...
    const onDeleteChunk = async (chunk) => {
        const confirmPayload = {
            title: `Удалить`,
            description: `Удалить фрагмент ${chunk.id}? Это действие нельзя будет отменить.`,
        ...
        if (isConfirmed) {
            ...
            try {
                ...
                if (delResp.data) {
                    enqueueSnackbar({
                        message: 'Фрагмент документа успешно удалён!',
                        options: {
                            key: new Date().getTime() + Math.random(),
                            variant: 'success',
                            action: (key) => (
                        ...
                        <div style={{ width: '100%' }}>
                            ...
                            >
                                <div style={{ marginRight: 20, display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                                    ...
                                    Показываются {Math.min(start, totalChunks)}–{end} из {totalChunks} фрагментов
                                    <IconButton
                                        size='small'
                                        onClick={() => changePage(currentPage + 1)}
                                        style={{ marginLeft: 10 }}
                                ...
                                <div style={{ marginRight: 20, display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                                    ...
                                    {getChunksApi.data?.characters?.toLocaleString()} символов
                                </div>
                            </div>
                        </div>
                        <div>
