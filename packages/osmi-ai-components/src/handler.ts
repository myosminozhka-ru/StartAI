...
class ExtendedLunaryHandler extends LunaryHandler {
    ...
    constructor({ OSMIOptions, ...options }: any) {
        super(options)
        this.appDataSource = OSMIOptions.appDataSource
        this.databaseEntities = OSMIOptions.databaseEntities
        this.chatId = OSMIOptions.chatId
        this.apiMessageId = OSMIOptions.apiMessageId
    }
...
export class AnalyticHandler {
    ...
    async onChainStart(name: string, input: string, parentIds?: ICommonObject) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.handlers, 'arize')) {
            ...
            if (!parentIds || !Object.keys(parentIds).length) {
                rootSpan = tracer ? tracer.startSpan('OSMI') : undefined
                if (rootSpan) {
