...
import { OSMI_CHATID, getBaseClasses, getCredentialData, getCredentialParam } from '../../../src/utils'
...
class Upstash_VectorStores implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (upstashMetadataFilter) obj.filter += ` OR ${OSMI_CHATID} = "${options.chatId}" OR HAS NOT FIELD ${OSMI_CHATID}`
            else obj.filter = `${OSMI_CHATID} = "${options.chatId}" OR HAS NOT FIELD ${OSMI_CHATID}`
        }
