...
class Milvus_VectorStores implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (milvusFilter) milvusFilter += ` OR ${OSMI_CHATID} == "${options.chatId}" OR NOT EXISTS(${OSMI_CHATID})`
            else milvusFilter = `${OSMI_CHATID} == "${options.chatId}" OR NOT EXISTS(${OSMI_CHATID})`
        }
