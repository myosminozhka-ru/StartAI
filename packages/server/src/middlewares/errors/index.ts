...
async function errorHandlerMiddleware(err: InternalStartAIError, req: Request, res: Response, next: NextFunction) {
    const statusCode = err.statusCode || StatusCodes.INTERNAL_SERVER_ERROR
    if (err.message.includes('401 Incorrect API key provided'))
        err.message = '401 Неверный ключ модели или неправильная конфигурация локальной модели.'
    let displayedError = {
        statusCode,
        success: false,
        message: err.message,
