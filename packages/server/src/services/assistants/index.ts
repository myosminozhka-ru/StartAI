...
const createAssistant = async (requestBody: any, orgId: string): Promise<Assistant> => {
    try {
        ...
        if (!requestBody.details) {
            throw new InternalStartAIError(StatusCodes.INTERNAL_SERVER_ERROR, `Invalid request body`)
        }
        ...
        if (requestBody.type === 'CUSTOM') {
            ...
            appServer.metricsProvider?.incrementCounter(STARTAI_METRIC_COUNTERS.ASSISTANT_CREATED, {
        ...
        try {
            ...
            if (!credential) {
                throw new InternalStartAIError(StatusCodes.NOT_FOUND, `Credential ${requestBody.credential} not found`)
            }
            ...
            if (!openAIApiKey) {
                throw new InternalStartAIError(StatusCodes.NOT_FOUND, `OpenAI ApiKey not found`)
            }
            ...
            throw new InternalStartAIError(StatusCodes.INTERNAL_SERVER_ERROR, `Error creating new assistant - ${getErrorMessage(error)}`)
        }
        const newAssistant = new Assistant()
        Object.assign(newAssistant, requestBody)
        ...
        appServer.metricsProvider?.incrementCounter(STARTAI_METRIC_COUNTERS.ASSISTANT_CREATED, { status: STARTAI_COUNTER_STATUS.SUCCESS })
...
const deleteAssistant = async (assistantId: string, isDeleteBoth: any): Promise<DeleteResult> => {
    try {
        ...
        if (!assistant) {
            throw new InternalStartAIError(StatusCodes.NOT_FOUND, `Assistant ${assistantId} not found`)
        }
        ...
        try {
            ...
            if (!credential) {
                throw new InternalStartAIError(StatusCodes.NOT_FOUND, `Credential ${assistant.credential} not found`)
            }
            ...
            throw new InternalStartAIError(StatusCodes.INTERNAL_SERVER_ERROR, `Error deleting assistant - ${getErrorMessage(error)}`)
        }
    } catch (error) {
        throw new InternalStartAIError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const updateAssistant = async (assistantId: string, requestBody: any): Promise<Assistant> => {
    try {
        ...
        try {
            ...
            if (!credential) {
                throw new InternalStartAIError(StatusCodes.NOT_FOUND, `Credential ${body.credential} not found`)
            }
            ...
            throw new InternalStartAIError(StatusCodes.INTERNAL_SERVER_ERROR, `Error updating assistant - ${getErrorMessage(error)}`)
        }
    } catch (error) {
        throw new InternalStartAIError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const getTools = async (): Promise<any> => {
    try {
        ...
        throw new InternalStartAIError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: assistantsService.getTools - ${getErrorMessage(error)}`)
    }
}
...
const generateAssistantInstruction = async (task: string, selectedChatModel: ICommonObject): Promise<ICommonObject> => {
    try {
        ...
        if (selectedChatModel && Object.keys(selectedChatModel).length > 0) {
            ...
            const nodeData = {
                credential: selectedChatModel.credential || selectedChatModel.inputs['STARTAI_CREDENTIAL_ID'] || undefined,
