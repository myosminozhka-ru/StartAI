...
const getAllTemplates = async () => {
    try {
        ...
        const OSMIDocsQnAIndex = sortedTemplates.findIndex((tmp) => tmp.templateName === 'Docs QnA')
        if (OSMIDocsQnAIndex > 0) {
            sortedTemplates.unshift(sortedTemplates.splice(OSMIDocsQnAIndex, 1)[0])
        }
