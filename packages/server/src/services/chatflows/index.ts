...
export function validateChatflowType(type: ChatflowType | undefined) {
    if (!Object.values(EnumChatflowType).includes(type as EnumChatflowType))
        throw new InternalStartAIError(StatusCodes.BAD_REQUEST, ChatflowErrorMessage.INVALID_CHATFLOW_TYPE)
}
...
const getChatflowByApiKey = async (apiKeyId: string, keyonly?: unknown): Promise<any> => {
    try {
        ...
        if (dbResponse.length < 1) {
            throw new InternalStartAIError(StatusCodes.NOT_FOUND, `Chatflow not found in the database!`)
        }
        return dbResponse
    } catch (error) {
        throw new InternalStartAIError(
...
const getChatflowById = async (chatflowId: string): Promise<any> => {
    try {
        ...
        if (!dbResponse) {
            throw new InternalStartAIError(StatusCodes.NOT_FOUND, `Chatflow ${chatflowId} not found in the database!`)
        }
        return dbResponse
    } catch (error) {
        throw new InternalStartAIError(
...
const saveChatflow = async (
    ...
    appServer.metricsProvider?.incrementCounter(
        dbResponse?.type === 'MULTIAGENT' ? STARTAI_METRIC_COUNTERS.AGENTFLOW_CREATED : STARTAI_METRIC_COUNTERS.CHATFLOW_CREATED,
    ...
    try {
        ...
        if (dbResponse.chatbotConfig || uploadsConfig) {
            try {
                ...
                throw new InternalStartAIError(StatusCodes.INTERNAL_SERVER_ERROR, `Error parsing Chatbot Config for Chatflow ${chatflowId}`)
            }
        }
        return 'OK'
    } catch (error) {
