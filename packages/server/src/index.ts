...
import OSMIApiV1Router from './routes'
...
export class App {
    ...
    async config() {
        ...
        const OSMI_file_size_limit = process.env.OSMI_FILE_SIZE_LIMIT || '50mb'
        this.app.use(express.json({ limit: OSMI_file_size_limit }))
        this.app.use(express.urlencoded({ limit: OSMI_file_size_limit, extended: true }))
        ...
        this.app.use('/api/v1', OSMIApiV1Router)
        ...
        this.app.get('/api/v1/ip', (request, response) => {
            response.send({
                ip: request.ip,
                msg: 'Check returned IP address in the response. If it matches your current IP address ( which you can get by going to http://ip.nfriedly.com/ or https://api.ipify.org/ ), then the number of proxies is correct and the rate limiter should now work correctly. If not, increase the number of proxies by 1 and restart Cloud-Hosted OSMI until the IP address matches your own. Visit https://docs.OSMIai.com/configuration/rate-limit#cloud-hosted-rate-limit-setup-guide for more information.'
            })
        })
    ...
    async stopApp() {
        try {
            ...
            logger.error(`❌[server]: OSMI Server shut down error: ${e}`)
        }
    }
}
