...
export const executeUpsert = async ({
    ...
    if (vsNodesWithFileUpload.length > 1) {
        throw new InternalOsmiError(StatusCodes.INTERNAL_SERVER_ERROR, 'Multiple vector store nodes with fileUpload enabled')
    ...
    if (vsNodes.length > 1 && !stopNodeId) {
        ...
        throw new InternalOsmiError(StatusCodes.NOT_FOUND, 'No vector node found')
    }
...
export const upsertVector = async (req: Request, isInternal: boolean = false) => {
    ...
    try {
        ...
        if (process.env.MODE === MODE.QUEUE) {
            ...
            appServer.metricsProvider?.incrementCounter(OSMI_METRIC_COUNTERS.VECTORSTORE_UPSERT, {
        ...
        appServer.metricsProvider?.incrementCounter(OSMI_METRIC_COUNTERS.VECTORSTORE_UPSERT, { status: OSMI_COUNTER_STATUS.FAILURE })
