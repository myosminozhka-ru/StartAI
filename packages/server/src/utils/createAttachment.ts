...
export const createFileAttachment = async (req: Request) => {
    ...
    if (!chatflowid || !isValidUUID(chatflowid)) {
        throw new InternalOsmiError(StatusCodes.BAD_REQUEST, 'Invalid chatflowId format - must be a valid UUID')
    }
    if (isPathTraversal(chatflowid)) {
        throw new InternalOsmiError(StatusCodes.BAD_REQUEST, 'Invalid path characters detected')
    }
    ...
    if (!fileUploadEnabled) {
        throw new InternalOsmiError(StatusCodes.BAD_REQUEST, 'File upload is not enabled for this chatflow')
    }
