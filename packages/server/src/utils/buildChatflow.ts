...
import { STARTAI_METRIC_COUNTERS, STARTAI_COUNTER_STATUS, IMetricsProvider } from '../Interface.Metrics'
...
const incrementSuccessMetricCounter = (metricsProvider: IMetricsProvider, isInternal: boolean, isAgentFlow: boolean) => {
    if (isAgentFlow) {
        metricsProvider?.incrementCounter(
            isInternal ? STARTAI_METRIC_COUNTERS.AGENTFLOW_PREDICTION_INTERNAL : STARTAI_METRIC_COUNTERS.AGENTFLOW_PREDICTION_EXTERNAL,
            { status: STARTAI_COUNTER_STATUS.SUCCESS }
        )
    } else {
        metricsProvider?.incrementCounter(
            isInternal ? STARTAI_METRIC_COUNTERS.CHATFLOW_PREDICTION_INTERNAL : STARTAI_METRIC_COUNTERS.CHATFLOW_PREDICTION_EXTERNAL,
            { status: STARTAI_COUNTER_STATUS.SUCCESS }
        )
    }
}
