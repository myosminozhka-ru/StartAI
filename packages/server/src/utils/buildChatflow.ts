...
import { OSMI_METRIC_COUNTERS, OSMI_COUNTER_STATUS, IMetricsProvider } from '../Interface.Metrics'
...
export const utilBuildChatflow = async (req: Request, isInternal: boolean = false): Promise<any> => {
    ...
    const isTool = req.get('OSMI-tool') === 'true'
    const isEvaluation: boolean = req.headers['X-OSMI-Evaluation'] || req.body.evaluation
    let evaluationRunId = ''
    ...
    try {
        ...
        if (e instanceof InternalOsmiError && e.statusCode === StatusCodes.UNAUTHORIZED) {
            throw e
        } else {
...
const incrementSuccessMetricCounter = (metricsProvider: IMetricsProvider, isInternal: boolean, isAgentFlow: boolean) => {
    if (isAgentFlow) {
        metricsProvider?.incrementCounter(
            isInternal ? OSMI_METRIC_COUNTERS.AGENTFLOW_PREDICTION_INTERNAL : OSMI_METRIC_COUNTERS.AGENTFLOW_PREDICTION_EXTERNAL,
            { status: OSMI_COUNTER_STATUS.SUCCESS }
        )
    } else {
        metricsProvider?.incrementCounter(
            isInternal ? OSMI_METRIC_COUNTERS.CHATFLOW_PREDICTION_INTERNAL : OSMI_METRIC_COUNTERS.CHATFLOW_PREDICTION_EXTERNAL,
            { status: OSMI_COUNTER_STATUS.SUCCESS }
        )
    }
}
...
const incrementFailedMetricCounter = (metricsProvider: IMetricsProvider, isInternal: boolean, isAgentFlow: boolean) => {
    if (isAgentFlow) {
        metricsProvider?.incrementCounter(
            isInternal ? OSMI_METRIC_COUNTERS.AGENTFLOW_PREDICTION_INTERNAL : OSMI_METRIC_COUNTERS.AGENTFLOW_PREDICTION_EXTERNAL,
            { status: OSMI_COUNTER_STATUS.FAILURE }
        )
    } else {
        metricsProvider?.incrementCounter(
            isInternal ? OSMI_METRIC_COUNTERS.CHATFLOW_PREDICTION_INTERNAL : OSMI_METRIC_COUNTERS.CHATFLOW_PREDICTION_EXTERNAL,
