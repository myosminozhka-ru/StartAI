...
export const getEndingNodes = (
    ...
    let error: InternalOsmiError | null = null
    for (const endingNode of endingNodes) {
        const endingNodeData = endingNode.data
        if (!endingNodeData) {
            error = new InternalOsmiError(StatusCodes.INTERNAL_SERVER_ERROR, `Ending node ${endingNode.id} data not found`)
    ...
    if (endingNodes.length === 0 || error === null) {
        error = new InternalOsmiError(StatusCodes.INTERNAL_SERVER_ERROR, `Ending nodes not found`)
    }
    ...
    for (const node of reactFlowNodes) {
        ...
        if (sessionId && node.data.inputs) {
            if (node.data.type === 'OpenAIAssistant') {
                ...
                const initializedInstance: OsmiMemory = await newNodeInstance.init(node.data, '', options)
...
export const getVariableValue = async (
    ...
    while (startIdx < endIdx) {
        ...
        if (substr === '}}' && variableStack.length > 0 && variableStack[variableStack.length - 1].substr === '{{') {
             ...
             * FROM: hello i am ben\n\n\thow are you?
             * TO: hello i am benOSMI_NEWLINEOSMI_NEWLINEOSMI_TABhow are you?
             */
...
export const decryptCredentialData = async (
    ...
    if (USE_AWS_SECRETS_MANAGER && secretsManagerClient) {
        try {
            if (encryptedData.startsWith('OSMICredential_')) {
...
export const getSessionChatHistory = async (
    ...
    const initializedInstance: OsmiMemory = await newNodeInstance.init(memoryNode.data, '', {
        chatflowid,
        appDataSource,
        databaseEntities,
        logger
