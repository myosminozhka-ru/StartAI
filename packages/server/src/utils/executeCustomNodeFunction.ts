import { handleEscapeCharacters, ICommonObject } from 'osmi-ai-components'
    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(componentNodes, 'customFunction')) {
            try {
                ...
                throw new InternalOsmiError(StatusCodes.INTERNAL_SERVER_ERROR, `Error running custom function: ${error}`)
            }
        } else {
            throw new InternalOsmiError(StatusCodes.NOT_FOUND, `Node customFunction not found`)
        }
    } catch (error) {
