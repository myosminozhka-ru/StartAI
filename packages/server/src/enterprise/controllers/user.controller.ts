...
export class UserController {
    ...
    public async update(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (!currentUser) {
                throw new InternalOsmiError(StatusCodes.UNAUTHORIZED, UserErrorMessage.USER_NOT_FOUND)
            }
            const { id } = req.body
            if (currentUser.id !== id) {
                throw new InternalOsmiError(StatusCodes.FORBIDDEN, UserErrorMessage.USER_NOT_FOUND)
            }
